...
class RobertaSelfAttention_Pairwise(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(
            config, "embedding_size"
        ...
        self.query = nn.Linear(config.hidden_size, self.all_head_size)
        self.pairwise_factors = nn.Parameter(torch.randn((self.num_attention_heads, self.attention_head_size, self.attention_head_size)))
        self.value = nn.Linear(config.hidden_size, self.all_head_size)
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            key_layer = self.transpose_for_scores(self.query(encoder_hidden_states))
            value_layer = self.transpose_for_scores(self.value(encoder_hidden_states))
            attention_mask = encoder_attention_mask
        elif past_key_value is not None:
            key_layer = self.transpose_for_scores(self.query(hidden_states))
            value_layer = self.transpose_for_scores(self.value(hidden_states))
            key_layer = torch.cat([past_key_value[0], key_layer], dim=2)
            value_layer = torch.cat([past_key_value[1], value_layer], dim=2)
        else:
        ...
        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = (query_layer @ self.pairwise_factors @ key_layer.transpose(-1, -2))
...
class RobertaSelfAttention_Symmetric(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        if config.hidden_size % config.num_attention_heads != 0 and not hasattr(
            config, "embedding_size"
...
class RobertaAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        match config.plpi_affinity_function:
            case "vanilla":
                self.self = RobertaSelfAttention(config, position_embedding_type=position_embedding_type)
            case "symmetric":
                self.self = RobertaSelfAttention_Symmetric(config, position_embedding_type=position_embedding_type)
            case "pairwise":
                self.self = RobertaSelfAttention_Pairwise(config, position_embedding_type=position_embedding_type)
            case _:
                self.self = RobertaSelfAttention(config, position_embedding_type=position_embedding_type)
